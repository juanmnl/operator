...
// prose answers (headings, fenced code, bullet/ordered lists, blockquotes, rules, and
// inline bold/italic/code/links). It is deliberately NOT full CommonMark — nested lists
// collapse by indent depth, tables are left as raw paragraph text (a later phase). Being
// pure (string in → data out), it's unit-testable without a canvas.
...
export type Block =
  ...
  | { type: 'list'; ordered: boolean; index: number; depth: number; spans: Span[] }
...
export function parseBlocks(md: string): Block[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (ul) {
      flushPara()
      blocks.push({ type: 'list', ordered: false, index: 0, depth: indentDepth(ul[1]), spans: parseInline(ul[2]) })
      continue
    }
