...
/// The preview inspector's injected script, from `src/shared/preview-inspector.js`.
///
/// A FILE RATHER THAN A STRING LITERAL because it is no longer Rust's alone: the Electron
/// shell injects the same script, and two copies of 137 lines of DOM code would diverge on
/// the first fix. `include_str!` keeps this build byte-identical to what it was.
const INSPECTOR_JS: &str = include_str!("../../src/shared/preview-inspector.js");
